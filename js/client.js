...
socket.on('board', function(board_data) {
  ...
  var adjustBoardSizeToFit = function() {
    ...
    $('#board').css('width', (squareSize + ((squareSize / 50) * 2)) * board_data.dimensions.width);
...
socket.on('lose', function(info) {
  ...
  Materialize.toast(loser + ' hit a mine!', 5000, 'red');
});
socket.on('win', function(winners) {
  Materialize.toast(winners + ' won the game!', 5000, 'green');
});
